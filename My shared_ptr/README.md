Реализация класса `shared_ptr`, который является умным указателем на матрицу, управляющим ее временем жизни.
`shared_ptr` позволяет копирование и присваивание. Копии указывают на ту же матрицу, что и оригинал.
Матрица реализована в классе `Matrix`.
...
Для реализации такой логики используйтся подсчет ссылок.
